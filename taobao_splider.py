...
import pymysql
import licence
import jingdong_splider
import suning_requests
...
def get_taobao(project, key_name, key_tag, key_platform, key_place, match):
    '''
    主函数，负责协调各函数
    :return:
    '''
    ...
    for keys in key_name:
        ...
        key = keys.split(':')[1]
        # 查询搜索商品的所有数据
        dataList = searchTaobaoKey(key, num)
        dates = getLastDate()  # 获取时间
        goodsList = taobaoGoodsUrl(dataList)  # 获取所有商品数据
        if not goodsList:
            print('数据为空，请检查cookie是否失效')
            continue
        for i in goodsList:
            try:
...
if __name__ == "__main__":
    ...
    for i in range(len(project)):
        for platform in key_platform[i]:
            if platform == '淘宝':
                # print(platform)
                get_taobao(project[i], key_name[i], key_tag[i], platform, key_place[i], search[i])
            elif platform == '京东':
                jingdong_splider.get_jd(project[i], key_name[i], key_tag[i], platform, key_place[i], search[i])
                # print(platform)
            elif platform == '苏宁':
                suning_requests.get_suning(project[i], key_name[i], key_tag[i], platform, key_place[i], search[i])
                # print(platform)
            elif platform == '拼多多':
                # print(platform)
                pdd_splider.get_pdd('127.0.0.1:9222', project[i], key_name[i], key_tag[i], platform, key_place[i], search[i])
    # 调用检测函数
    # licence.run()
    # 调用查询函数对当日数据进行留证
    # select_mysql()