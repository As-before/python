...
def select_mysql(date):
    ...
    cursor = db1.cursor()
    # sql语句
    sql = "select * from object where dates like '{}%'".format(date)
    # 查询
    cursor.execute(sql)
    # 获取全部数据
...
def select_goods_mysql(url_md5):
    ...
    cursor = db1.cursor()
    # sql语句
    sql = "select * from object where tag2='{}' order by id".format(url_md5)
    # 查询
    cursor.execute(sql)
    # 获取全部数据
    results = cursor.fetchall()
...
def run():
    ...
    # 将数据入库
    putLibrary('licence_object', newList, changesList)
